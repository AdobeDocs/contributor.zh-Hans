...
- *分支*：所有存放庫皆包含一個預設分支（通常名為「主要」），以及一或多個會合併回主要分支的分支。 主要分支是發佈內容的目前版本和來源。 它是创建存储库中所有其他分支的父级存储库。
...
專案的預設/主要分支是專案內容的目前版本。 主要分支中的內容（以及從中建立的分支）會與文章主題的組織方式一致。 子目录用于组织内容和图像资产。
