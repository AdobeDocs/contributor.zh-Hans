...
description: 了解如何对Adobe文档进行Experience League。
...
从 [Git和GitHub基础知识](git-fundamentals.md) Git存储库包含主分支，以及尚未集成到主存储库中的任何其他进程中的分支。 每当您引入一组逻辑上相关的更改时，最佳做法是创建 *工作分支* 以通过工作流管理更改。 我们在此将其称为工作分支，因为它是一个用于迭代/优化更改的工作区，直到它们可以集成回主分支为止。

通过隔离特定分支的相关更改，您可以单独控制和引入这些更改，将这些更改定位到发布周期中的特定发布时间。实际上，根据您所从事的工作类型，您可以轻松地在存储库中使用多个工作分支。在同一时间使用多个分支并不罕见，各个分支代表各个不同的项目。

>[!NOTE]
>
>在主分支中进行更改 *不是好做法*. 假设您使用主分支为定时功能发布引入一组更改。 您已完成更改并正等待发布它们。在此期间，您会收到一个紧急请求，要求修复某些内容，因此您对主分支中的文件进行更改，然后发布更改。 在此示例中，您既无意中发布了修复&#x200B;*，又发布了*&#x200B;您原本计划在特定日期发布的更改。
...
您可以提交建议的更改，方法是通过将它们捆绑在添加到目标存储库的 PR 队列的新拉取请求 (PR) 中。拉取请求会启用 GitHub 的协作模型，方法是请求将工作分支的更改拉入并合并到另一个分支。在大多数情况下，其他分支是主存储库中的默认/主分支。
